...
class Solution {
    ...
    public boolean solution(int[] arrA, int[] arrB) {
        if(arrA.length != arrB.length)
            return false;
        if(func_b(arrA, arrB)){	// 추가	
            int[] arrAtemp = func_a(arrA);	// 추가
            if(func_c(arrAtemp, arrB))	// 추가
                return true;
        }
        return false;
    }
...
public int solution(String[] bishops) {
    ...
    for(String i : bishops) {
    	int row = i.charAt(0)-'A';
    	int col = Character.getNumericValue(i.charAt(1)) - 1;
    	int cnt = 0;
    	...
    	for(int len=0;len<7;len++) {
    		for(int dir=0;dir<4;dir++) {
    			int nx = row + len * dx[dir];
    			int ny = col + len * dy[dir];
...
public int solution(String s1, String s2) {
    ...
    int max = 0;
    for(int i=1;i<len;i++) {
    	if(s1.substring(0,i).equals(s2.substring(s2.length()-1-i)))
    		max = Math.max(max, i);
    }
    
    for(int i=1;i<len;i++) {
    	if(s2.substring(0,i).equals(s1.substring(s1.length()-i)))
    		max = Math.max(max, i);
    }
    
    answer = s1.length() + s2.length() - max;
    ...
    for (int i = 0; i < primeLen - 2; i++)
        for (int j = i + 1; j < primeLen - 1; j++)
            for (int k = j + 1; k < primeLen; k++)
                if (primes.get(i) + primes.get(j) + primes.get(k) == n)	// 추가
                    answer++;
    return answer;
}
```
...
class HairShop extends Shop {	// 추가
    public HairShop(){
        super();
    }
    
    public boolean reserve(Customer customer){	// 추가
        if(customer.numOfPeople != 1)	// 추가
            return false;
        
        Iterator<Customer> iter = reserveList.iterator();
        while (iter.hasNext()) {
            Customer r = iter.next();
            if(r.time == customer.time)	// 추가
                return false;
        }
        reserveList.add(customer);
        return true;
...
class Restaurant extends Shop {	// 추가
    public Restaurant(){
        super();
    }
    
    public boolean reserve(Customer customer){
        if(customer.numOfPeople < 2 || customer.numOfPeople > 8)	// 추가
            return false;
        int count = 0;
        ...
        while (iter.hasNext()) {
            Customer r = iter.next();
            if(customer.numOfPeople == r.numOfPeople)	// 추가
                count += 1;
        }
        if(count >= 2)
            return false;
